...
$.fn.fullCalendar = function(options) {
	...
	this.each(function(i, _element) { // loop each DOM element involved
		...
		// a new calendar initialization
			...
			// console.log("#171开始,calendar.render() 开始");
			calendar.render();
			// console.log("结束了,在循环以外.#172")
		}
	});
...
Grid.mixin({
	...
	renderEvents: function(events) {
		...
		var fgSegs = [];
		var i, seg;
		// console.log("this.eventsToSegs:这是将总对像传递过去,返回对应月需要的对象");
		// console.log("1segs.length:"+segs.length);
		...
		// console.log("fgSegs.length:"+fgSegs.length);
		// console.log("bgSegs.length:"+bgSegs.length);
		// console.log("======================");
		var tmp = "";
		for (var j =  0; j < events.length; j++) {
			tmp += this.fgSegHtml2(events[j]);
		};
		this.yearA(tmp);
	},

	fgSegHtml2: function(seg, disableResizing) {
		var view = this.view;
		var event = seg;
		var skinCss = "background-color:"+seg.color+";border-color:"+seg.color+";";
		var timeHtml = '';
		var titleHtml;
		var endT = event.endT?new Date(event.endT):new Date(event.end)
		titleHtml =
			'<span class="fc-title spanBG" style="' + skinCss + ';">' +
				(htmlEscape(event.title || '') || '&nbsp;') + // we always want one line of height
			'</span>';
		return '<a data-start="'+new Date(event.start)+'" data-end="'+endT+'" title="'+ (event.linkTitle || '' || '&nbsp;') +'" class="fc-day-grid-event fc-event fc-not-start fc-end"'  +
				(event.url ?
					' href="' + htmlEscape(event.url) + '"' :
					' style="background:none; border:0;position:relative; margin:0;"') +'>' +
				'<div class="fc-content">' + 
				titleHtml + "<span class='spanLine' style='"+skinCss+"'></span><i class='rightPT' style='"+skinCss+"'></i>" +
				'</div>' +
			'</a>';
	},
	...
	setYear : function(){
		var $yearDate = $("#fc-yearDate"),
			$quarter = $("#quarter"),
			thisYear = $(".fc-toolbar h2").text(),
			thidTD = $quarter.find("td");
		$yearDate.find("a").each(function(){
			var $this = $(this),
				s = new Date($this.data("start")),
				startY = s.getFullYear();
				startM = s.getMonth()+1 ,
				startD = s.getDate(),
				e = new Date($this.data("end")),
				endY = e.getFullYear();
				endM = e.getMonth()+1;
				endD = e.getDate();

			var marginLeft = 0,thisWidth = 0;
			for(var i = 1; i<startM; i++) {
				marginLeft+=thidTD.eq(i-1).outerWidth(true);
			}
			var Lw = startD/30*thidTD.eq(startM-1).outerWidth(true);
				marginLeft+= Lw;
				nextSpanOBJ = $(this).find(".fc-title");

			if(startY!=thisYear&&endY!=thisYear) {$this.remove();}
			...
			if(startY==endY){
				...
				if(startM>=endM) {
					// 月间隔很短的
					var nextSpan = nextSpanOBJ.width(),
						tdWidth = thidTD.eq(1).outerWidth(true)*0.2;
					if(nextSpan>50) {
						nextSpanOBJ.addClass("limit").width(tdWidth);
						nextSpan = tdWidth;
					}
					thisWidth = nextSpan;
				} else {
					// 月间隔很长的
					thisWidth += thidTD.eq(startM-1).outerWidth(true)-Lw;
					for (var i = startM; i<endM; i++) {
						thisWidth+=thidTD.eq(i-1).outerWidth(true);
					}
					var tdENDW = thidTD.eq(endM-1).outerWidth(true),
						Ew = endD/30*tdENDW,
						tdW = thidTD.eq(1).width();
					thisWidth -= tdENDW-Ew;
					if(nextSpanOBJ.width()>tdW) {
						nextSpanOBJ.addClass("limit").width(tdW-8);
					}
				}
			} else if(startY!=thisYear&&endY==thisYear) {
				// 结束年是当年,开始不是当年
				marginLeft=0;
				startM = 1;
				if(startM>=endM) {
					...
					// 月间隔很长的
					nextSpanOBJ = $(this).find(".fc-title");
					thisWidth += thidTD.eq(startM-1).outerWidth(true);
					for (var i = startM; i<endM; i++) {
						thisWidth+=thidTD.eq(i-1).outerWidth(true);
					}
					var tdENDW = thidTD.eq(endM-1).outerWidth(true),
						Ew = endD/30*tdENDW,
						objW = nextSpanOBJ.width(),
						tdW = thidTD.eq(1).width();
					thisWidth -= tdENDW-Ew;
					if(objW>tdW) {
						nextSpanOBJ.addClass("limit").width(tdW-8);
				...
				if(startM>=endM) {
					...
					var tdENDW = thidTD.eq(endM-1).outerWidth(true);
						// Ew = endD/30*tdENDW;
					// thisWidth -= tdENDW;
					if(nextSpanOBJ.width()>thisWidth) {
						nextSpanOBJ.addClass("limit").width(thidTD.eq(1).width()-8);
					}
				}

			}
			$this.css({"marginLeft":marginLeft,"width":thisWidth})
		})
	},
	...
	renderFgSegEls: function(segs, disableResizing) {
		...
		if (segs.length) { // don't build an empty html string
			// console.log("3segs.length:"+segs.length);
			// console.log("这里已经开始拼接月值了.");
	...
	eventsToSegs: function(events, rangeToSegsFunc) {
		...
		// console.log("eventsToSegs:事件,计算返回的对象");
		// console.log(eventRanges);

		// console.log("eventRanges:"+eventRanges);
		// console.log("eventRanges.length:"+eventRanges.length);

		for (i = 0; i < eventRanges.length; i++) {
			// console.log(this.eventRangeToSegs(eventRanges[i], rangeToSegsFunc));
			segs.push.apply(
				segs,
	...
	eventRangeToSegs: function(eventRange, rangeToSegsFunc) {
		var segs;
		var i, seg;
		// console.log("eventRangeToSegs: 把所有对象循环一遍,9个对象9遍,每个对象在当前月中要展现几次");
		// console.log("eventRange,传递进来的对象");
		...
		// console.log("eventRangeToSegs:"+segs.length);
...
var DayGrid = Grid.extend({
	...
	rangeToSegs: function(range) {
		...
		// console.log("==0293982398723:segs:"+seg);
		// console.log(seg);
...
DayGrid.mixin({
	...
	renderFgSegs: function(segs) {
		var rowStructs;
		var obj = this;
		// console.log("2segs.length:"+segs.length);
...
// obj.yearA(rowStructs[1].tbodyEl);
// console.log("每月多了那么几个值,就是他:rowStructs:"+rowStructs);
// console.log(rowStructs);

		// append to each row's content skeleton
		// console.log("rowStructs.length:"+rowStructs.length);

		this.rowEls.each(function(i, rowNode) {
			console.log("each 里面<:></:>this.rowEls.each i:"+i);
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a data-start="'+event.start+'" data-end="'+event.end+'" title="'+ (event.linkTitle || '' || '&nbsp;') +'" class="' + classes.join(' ') + '"' +
				(event.url ?
					' href="' + htmlEscape(event.url) + '"' :
					' style="background:none; border:0;position:relative; margin:0;"'
					) +
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a title="' +event.linkTitle + '" class="' + classes.join(' ') + '"' +
			(event.url ?
				' href="' + htmlEscape(event.url) + '"' :
				''
				) +
...
var View = fc.View = Class.extend({
	...
	renderViewEvents: function(events) {
		var obj = this;
		// console.log(events);
		// events 指所有的events 属性
		// console.log("#6700行,这里才1遍");
...
fcViews.year = View.extend({
	...
	renderYear: function(yearColumns, colCnt, showNumbers) {
		...
		this.buildSkeleton(this.yearColumns, showNumbers);   //年线
		this.buildDayGrids();
		this.updateCells();
		// console.log("10129行");
		// this.setYear();
	...
	buildSkeleton: function(monthsPerRow, showNumbers) {
		...
		var s, headerClass = this.tm + "-widget-header",tb;
		var weekNames = [];
		...
		// tb = '<table class="fc-year-main-table fc-border-separate" style="width:100%;"><tr>';
		// tb += '<td class="fc-year-month-border fc-first"></td>'; 

		// this.table = $(s).appendTo(this.el);

		// 新增
		s ='<table class="fc-year-main-table" style="width:100%;">';
		s+='<thead><tr><th colspan="3">第一季度Q1</th><th colspan="3">第二季度Q2</th><th colspan="3">第三季度Q3</th><th colspan="3">第四季度Q4</th></tr></thead>';
		s+='<thead><tr><th>1月</th><th>2月</th><th>3月</th><th>4月</th><th>5月</th><th>6月</th><th>7月</th><th>8月</th><th>9月</th><th>10月</th><th>11月</th><th>12月</th></tr></thead>';
		s+='<tbody><td colspan="12" class="fc-widget-content" style="border-left:0;border-right:0;">';
		s+='<div class="fc-day-grid-container fc-row" style="min-height:648px; margin-right:-2px;">'
		s+='<div class="fc-bg"><table style=""><tbody><tr id="quarter">'+
'<td class="fc-day fc-widget-content fc-past fc-first col" style="border-left:0;">&nbsp;</td>'+
'<td class="fc-day fc-widget-content fc-past" style="width:;">&nbsp;</td>'+
'<td class="fc-day fc-widget-content fc-past" style="width:;">&nbsp;</td>'+
'<td class="fc-day fc-widget-content fc-past" style="width:;">&nbsp;</td>'+
'<td class="fc-day fc-widget-content fc-past" style="width:;">&nbsp;</td>'+
'<td class="fc-day fc-widget-content fc-past" style="width:;">&nbsp;</td>'+
...
'<td class="fc-day fc-widget-content fc-past" style="width:;">&nbsp;</td>'+
'<td class="fc-day fc-widget-content fc-past" style="width:;">&nbsp;</td>'+
'<td class="fc-day fc-widget-content fc-past fc-last" style="border-right:0;">&nbsp;</td>'+
'</tr></tbody></table></div>';
		s+='<div id="fc-yearDate"></div>';
		s+='</div>';
